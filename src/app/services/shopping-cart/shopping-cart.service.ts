...
export class ShoppingCartService {
  ...
  totalCart = this.countCart.asObservable();
  ...
  getShoppingCart(id: number): Observable<ShoppingCartProduct[]> {
    return this.http.get<ShoppingCartProduct[]>(
      `${this.path}/shopping-cart/user/${id}`,
      { headers: this.getHeaders() }
    );
  }

  addShoppingCart(shoppingcart: ShoppingCartProduct[]) {
    return this.http.post<ShoppingCartProduct>(
  ...
  deleteShoppingCart(id: number) {
    return this.http.delete(`${this.path}/shopping-cart/${id}`, {
      headers: this.getHeaders(),
    });
  }
    ...
    if (this._auth.isAuthenticated()) {
      ...
      if (shoppingCart !== null) {
        for (let i = 0; i < shoppingCart.length; i++) {
          ...
          this.addShoppingCart(shoppingCart[i]).subscribe(
            (data) => {
              console.log(data);
            },
            (error) => {
