...
export class BranchesComponent implements OnInit {
  ...
  getBranches() {
    this._branch.getBranches().subscribe(
      (data) => {
  ...
  onBranchFormSubmit(form: any) {
    ...
    if (form.id > 0) {
      this._branch.putBranchOffice(id, restForm).subscribe(
        (data) => {
          this.getBranches();
      ...
      this._branch.postBranchOffice(restForm).subscribe(
        (data) => {
          this.getBranches();
          this._snackbar.openSnackBar(
  ...
  getProductsByBranch(branch_id: number) {
    ...
    this._branch.getProductsByBranch(branch_id).subscribe(
      (data) => {
        this.productsRows = data.filter((obj) =>
          obj.state === 'A' ? (obj.state = 'Activo') : (obj.state = 'Inactivo')
        );
      },
      (error) => {
        console.log(error);
  ...
  executeAction(obj: any) {
    ...
    if (obj.action === 'edit') {
      ...
      modalRef.componentInstance.eventEmitter.subscribe(
        (isDeleted: boolean) => {
          if (isDeleted) {
            this._branch.deleteBranchOffice(branch.id).subscribe(
              (data) => {
                this.getBranches();
                this._snackbar.openSnackBar(
  ...
  changeProductState(obj: any) {
    ...
    this._branch
      .putProductState(currentBranchId, productId, newState)
      .subscribe(
        (data) => {
