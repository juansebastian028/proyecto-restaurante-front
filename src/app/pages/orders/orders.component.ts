...
export class OrdersComponent implements OnInit {
  ...
  constructor(private _auth: AuthService,private _order: OrderService, private modalService: NgbModal, private _snackbar: SnackbarService) { }

  openModal(obj: any) {
    if(obj.action === "openModal"){
      const modalRef = this.modalService.open(ModalOrderComponent);
      this._order.getProductsOrder(obj.element.id).subscribe((data) => {
        ...
        modalRef.componentInstance.eventEmitter.subscribe((event:any) => {
          if(event.action === "finalize"){
            this._order.finalizeOrder(event.id, event.form).subscribe((data) => {
              this._snackbar.openSnackBar('Pedido finalizado exitosamente','bg-success','text-white');
              modalRef.close();
              this.refreshOrders();
            }, (error) => {
              this._snackbar.openSnackBar('Ocurrio un error al finalizar el pedido','bg-warning','text-white');
              console.log(error);
            });
          }else if(event.action === "cancel"){
            this._order.cancelOrder(event.id, event.form).subscribe((data) => {
              this._snackbar.openSnackBar('Pedido cancelado exitosamente','bg-success','text-white');
              modalRef.close();
              this.refreshOrders();
            }, (error) => {
              this._snackbar.openSnackBar('Ocurrio un error al cancelar el pedido','bg-warning','text-white');
              console.log(error);
            });
          }
         });
  ...
  getOrdersByBranch(id: number){
    this._order.getOrdersByBranch(id).subscribe((data) => {
      this.ordersRows = data;
    }, (error) => {
      console.log(error)
    });
  ...
  getOrdersByUser(id: number){
    this._order.getOrdersByUser(id).subscribe((data) => {
      this.ordersRows = data;
    }, (error) => {
      console.log(error)
    });
