...
export class ModalOrderComponent implements OnInit {
    ...
    this.order.products.forEach((product: any) => {
      ...
      product?.category?.modifier_groups.forEach((modifierGroup: any) => {
        modifierGroup?.modifiers.forEach((modifier: any) => {
          this.increaseTotal(modifier.unit_price_modifier, product.pivot.quantity);
        });
      });
    });
  }
  ...
  constructor(private fb: FormBuilder, private _auth: AuthService, public activeModal: NgbActiveModal) {
  ...
  onFormSubmit() {
    this.submitted = true;
    if(this.form.valid){
      this.eventEmitter.emit({id: this.order.id, form: this.form.value, action: 'cancel'});
    }
  }

  finalizeOrder(){
    this.eventEmitter.emit({id: this.order.id, form: { 'state': 'F' }, action: 'finalize'});
  }
